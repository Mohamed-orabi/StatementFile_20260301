...
// Branch 1
public class clsStatement_CommonCorpCompany : clsBasStatement
{
    private string strBankName, strProductCond;
    ...
    public clsStatement_CommonCorpCompany()
    {
    }
        ...
        try
        {
            ...
            foreach (DataRow currencyRow in DSstatement.Tables["currencyTable"].Rows)
            {
                ...
                foreach (DataRow contractRow in DSstatement.Tables["contractTable"].Rows)
                {
                    ...
                    cardsRows = DSstatement.Tables["tStatementMasterTable"].Select("contractno = '" + curContractNo + "' and accountcurrency = '" + curContarctCurr + "'"); //accountno
                    ...
                    {
                        masterRow = cardRow;
                        //>>if (curContarctCurr != contractRow[mAccountcurrency].ToString().Trim())
                        //>>{
                        //>>  if (curContarctCurr != string.Empty)
                        //>>  {
                        //>>    streamWrit.Flush();
                        ...
                        //>>    fileStrm.Close();
                        //>>  }
                        //>>  curContarctCurr = contractRow[mAccountcurrency].ToString().Trim();
                        //>>  strFileCompany = clsBasFile.getPathWithoutExtn(strFileName)
                        //>>    + "_" + curContarctCurr + "." + clsBasFile.getFileExtn(strFileName);
                        //>>  //curFileName = pStrFileName;
                        //>>  add2FileList(strFileCompany);
    ...
    private void printHeaderCompany()
    {
        ...
        streamWrit.WriteLine(basText.alignmentRight(masterRow[mCustomername], 50));  //
        streamWrit.WriteLine(basText.replicat(" ", 81) + masterRow[mCardbranchpartname]);  //
        streamWrit.WriteLine(basText.alignmentRight(ValidateArbic(masterRow[mCustomeraddress1].ToString()), 80)); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
        ...
        streamWrit.WriteLine(basText.alignmentRight(ValidateArbic(masterRow[mCustomeraddress2].ToString()), 80));
        streamWrit.WriteLine(basText.replicat(" ", 81) + pageNo + " / " + totalAccPages);  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
        streamWrit.WriteLine(basText.alignmentRight(ValidateArbic(masterRow[mCustomeraddress3].ToString().Trim()) + " " + masterRow[mCustomerregion].ToString().Trim() + " " + masterRow[mCustomercity].ToString().Trim(), 80));  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
    ...
    private void printDetailCompany()
    {
        ...
        //
        mainRows = DSstatement.Tables["tStatementMasterTable"].Select(mCardaccountno + " = '" + strStatementNo + "' and accountcurrency = '" + curContarctCurr + "'");
        ...
        streamWrit.WriteLine("  {0,16}  {1,-50} {2,16}", basText.formatCardNumber(curMainCard), masterRow[mCardclientname].ToString(), basText.formatNumUnSign(masterRow[mMindueamount], "#0.00", 16));   // {3,2}    basText.formatNumUnSign(masterRow[mClosingbalance],"#0.00",16),CrDb(Convert.ToDecimal(masterRow[mClosingbalance])) {3,16}  ,valOutstanding,valTransaction  basText.formatNumUnSign(masterRow[mMindueamount],"#0.00",16),{3,16} 
    }
    ...
    private void calcAccountRowsCompany()
    {
        ...
        accountRows = DSstatement.Tables["tStatementMasterTable"].Select("contractno = '" + curContractNo + "' and accountcurrency = '" + curContarctCurr + "'"); //accountno
        totalAccPages = 0;
        totAccRows = 0;
    ...
    private void printStatementSummary(string pCurrency)
    {
    ...
    ~clsStatement_CommonCorpCompany()
    {
        DSstatement.Dispose();
    }
}
