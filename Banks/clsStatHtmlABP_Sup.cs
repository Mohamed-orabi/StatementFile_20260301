...
public class clsStatHtmlABP_Sup : clsBasStatement
    {
        ...
        try
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
                ...
                if (prevcardNo != masterRow[mCardno].ToString())
                    ...
                    if (!string.IsNullOrEmpty(emailStr.ToString()) && string.IsNullOrEmpty(emailTo)) //emailStr != string.Empty && emailStr != null  emailTo != string.Empty && emailTo != null
                        {
                        streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + emailTo + "|" + emailRow["mobilephone"].ToString() + "|Without Email");
                        noOfWithoutEmails++;
    ...
    private void SendEmail(string pBody, string pSubject, string pTo)
    {
        try
        {
            ...
            {
                streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + pTo + "|" + emailRow["mobilephone"].ToString() + "|Bad Email");
                noOfBadEmails++;
            ...
            while (!sndMail.sendEmailAttachPicFile(strEmailFromTmp, pLstTo, pLstCC, pLstBCC, pLstAttachedPic, pLstAttachedFile, pSubject, pBody, clsCnfg.readSetting("SmtpServer")))
            {
                ...
                if (numOfTry > 100)
                {
                    ...
                    streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + emailTo + "|" + emailRow["mobilephone"].ToString() + "|Bad Email");
        ...
        {
            //clsBasErrors.catchError(ex);
            streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + pTo + "|" + emailRow["mobilephone"].ToString() + "|Bad Email " + pTo + " , Err Message >> " + ex.Message );
            noOfBadEmails++;
        }
        finally
        {
