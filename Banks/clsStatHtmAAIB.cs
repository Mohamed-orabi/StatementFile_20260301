...
public class clsStatHtmlAAIB : clsBasStatement
    {
        ...
        try
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
                ...
                if (prevAccountNo != masterRow[accountNoName].ToString())
                    ...
                    emailStr.Append("Password: " + strPrimaryCardNo.Substring(strPrimaryCardNo.Length - 4, 4));
                    totNoOfCardStat++;
                ...
                if (stmtNo == null)
                    stmtNo = masterRow[mStatementno].ToString();
                clsStatement_ExportRpt exportRptaaib = new clsStatement_ExportRpt();
                exportRptaaib.whereCond = "statementno = " + int.Parse(stmtNo);
                exportRptaaib.exportAttachment(pStrFileName, pBankCode, masterRow[mCardclientname].ToString() + "_" + masterRow[mAccountcurrency].ToString() + "_" + vCurDate.ToString("MMyyyy"), pReportName);
                exportRptaaib = null;
                stmtNo = null;
                stmtfilename = @clsBasFile.getPathWithoutFile(pStrFileName) +masterRow[mCardclientname].ToString().Replace('.', ' ').Replace('/', ' ').Replace(' ', '_') + "_" + masterRow[mAccountcurrency].ToString() + "_" + vCurDate.ToString("MMyyyy") + ".pdf";
                aryLstFilesStmt.Add(stmtfilename);
                SharpZip zip = new SharpZip();
                zip.createZip1(aryLstFilesStmt, @clsBasFile.getPathWithoutExtn(stmtfilename) + ".zip", strPrimaryCardNo.Substring(strPrimaryCardNo.Length - 4, 4));
                zip = null;
                aryLstFilesStmt.Remove(stmtfilename);
                clsBasFile.deleteFile(stmtfilename);
    ...
    protected void printHeader()
        ...
        emailStr.Append(@"</title>");
        emailStr.Append(@"</head><body  background='" + clsBasFile.getFileFromPath(strbackGround) + @"'><table border='0' width='100%' id='table1'>");
        ...
        //emailStr.Append(@"<a href='http://" + strbankWebLink + @"'><img border='0' src='logo.gif'></a></p></td>");
        ...
        //streamEmails.WriteLine(curAccountNumber + "|" + curClientID + "|" + pTo + "|" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"));
            ...
            if (HasAttachement)
                ...
                foreach (string fileName in Directory.GetFiles(attachedFilesStr))
                    {
                    if (fileName == clsBasFile.getPathWithoutExtn(stmtfilename) + ".zip")
                        {
            ...
            {
            //clsBasErrors.catchError(ex);
                streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + emailTo + "|" + (emailRow?["mobilephone"]?.ToString() ?? "no mobile number!") + "| Email " + pTo + " , Err Message >> " + ex.Message + ", Err Desc >> ");
            noOfBadEmails++;
            }
        finally
        {
