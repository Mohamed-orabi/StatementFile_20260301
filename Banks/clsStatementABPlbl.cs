...
// Branch 16
public class clsStatementABPlbl : clsBasStatement
{
    ...
    private string strEndOfAccount = "----------------- END OF STATEMENT -----------------";  //  + basText.replicat(" ",85) + "F 2"  + "\u000D"+ "M" ^L + ^M
    ...
    private int totNoOfCardStat, totNoOfPageStat, totNoOfCardEmailStat, totNoOfPageEmailStat;

    private int curCardRow = 0, curTotCardRows = 0, numOfErr = 0, totNoOfTransactions = 0, totNoOfTransEmail = 0, totNoOfTransactionsInt = 0, totNoOfTransEmailInt = 0;
        ...
        try
        {
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
            {
                ...
                if (prevAccountNo != masterRow[accountNoName].ToString())
                {
                    ...
                    if (totAccRows < 1
                      && Convert.ToDecimal(masterRow[mClosingbalance].ToString()) == 0) //cid             || (masterRow[mCardno].ToString() == curMainCard   // Convert.ToDecimal(
                    {
                        isHaveF3 = true;
        ...
        finally
        {
            if (preExit)
            {
                ...
                if (pStmntType == "Corporate")
                {
                    ...
                    clsStatement_CommonCorpCompany stmntABPCorp = new clsStatement_CommonCorpCompany();// + "NSGB_Business_Statement.txt"
                    stmntABPCorp.setFrm = frmMain;
                    if (pBankName == "ABP_Corporate")
                        //stmntABPCorp.productCond = "('Business Cardholder Contract (DUAL CCY)','Business Corporate Contract (DUAL CCY)','Business Cardholder Contract Silver (DUAL CCY)','Business Corporate Contract Silver (DUAL CCY)','Access Visa Cardholder Exec Contract (DUAL CCY)','Access Visa Corporate Exec Contract (DUAL CCY)','Additional Corporate Organization')";
                    ...
                    aryLstFilesCorp = stmntABPCorp.Statement(ppStrFileName, pBankName, pBankCode, pStrFile, pCurDate);// + "NSGB_Business_Statement.txt"
    ...
    protected void printHeader()
    {
        ...
        if (createCorporateVal)
            strmWriteCommon.WriteLine(basText.alignmentMiddle(masterRow[mCardclientname], 50));
        else
            strmWriteCommon.WriteLine(String.Empty);
        ...
        if (pageNo == 1)
            strmWriteCommon.WriteLine(basText.replicats(" ", 42) + basText.alignmentLeft("Previous Balance", 85) + basText.formatNumUnSign(masterRow[mOpeningbalance], "##0.00", 16) + " " + CrDb(Convert.ToDecimal(masterRow[mOpeningbalance])));  //+ CrDb(clsBasValid.validateStr(detailRow[dBilltranamountsign]))
        else
            strmWriteCommon.WriteLine(String.Empty);
    ...
    protected void printCardFooter()
    {
        ...
        if (isReward)
        {
            strmWriteCommon.WriteLine(String.Empty);
            strmWriteCommon.WriteLine(String.Empty);
            strmWriteCommon.WriteLine(basText.alignmentMiddle("RewardOpenBalance", 20) + basText.alignmentMiddle("RewardEarnedBonus", 20) + basText.alignmentMiddle("RewardRedeemedBonus", 20) + basText.alignmentMiddle("BonusAdjustment", 20) + basText.alignmentMiddle("RewardClosingBalance", 20));
            ...
            if (rewardRows.Length > 0)
            {
                ...
                strmWriteCommon.WriteLine(basText.formatNum(rewardRow[mOpeningbalance], "#0.00;(#0.00)", 20, "M") + basText.formatNum(rewardRow["EARNEDBONUS"], "#0.00;(#0.00)", 20, "M") + basText.formatNum(rewardRow["REDEEMEDBONUS"], "#0.00;(#0.00)", 20, "M") + basText.formatNum((rewardRow["bonusadjustment"].ToString() != "" ? Convert.ToDecimal(rewardRow["bonusadjustment"].ToString()) : Convert.ToDecimal(0.0)) * -1, "#0.00", 20, "M") + basText.formatNum(rewardRow[mClosingbalance], "#0.00;(#0.00)", 20, "M")); // + basText.formatNum((Convert.ToDecimal(rewardRow["bonusadjustment"]) * -1), "#0.00", 20, "L")  //strmWriteCommon.WriteLine(String.Empty);
            }
            else
            {
    ...
    private void calcAccountRows()
    {
        ...
        mainRows = DSstatement.Tables["tStatementMasterTable"].Select(accountNoName + " = '" + clsBasValid.validateStr(masterRow[accountNoName]) + "'");
        ...
        {
            curMainCard = CurCardNo;
            strmWriteErr.WriteLine("Account without main Card Number" + stmNo);
            numOfErr++;
        }
    }
