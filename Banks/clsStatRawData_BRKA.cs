...
// Branch X
public class clsStatRawData_BRKA : clsBasStatement
{
    ...
    protected string rewardCond;
    protected bool isReward;
    protected DataRow[]  rewardRows;
    protected DataRow rewardRow;

    public clsStatRawData_BRKA()
    {
    }
    ...
    public string Statement(string pStrFileName, DataSet pDSstatement)
    {
        try
        {
            ...
            string strFileHeader = "CardHolder Name" + fldSprtr + "Address1" + fldSprtr + "Address2" + fldSprtr + "Address3" +
              fldSprtr + "Address4" + fldSprtr + "Card Type" + fldSprtr+"Contract" +fldSprtr+ "Account" + fldSprtr + "Branch" + fldSprtr + "Statement Date" +
              ...
              fldSprtr + "Purch.Cash&Dr" + fldSprtr + "Card Management Fees" + fldSprtr + "Finance Charge" + fldSprtr + "External NO" + fldSprtr
              + "Card Primary" + fldSprtr + "Primary Card NO" + fldSprtr+ "EARNEDBONUS" + fldSprtr + "REDEEMEDBONUS" + fldSprtr + "EXPIREDBONUS" + fldSprtr + "CREDITCONTRACTS" + fldSprtr + "OVERDUE_DAYS" + fldSprtr+ "Expiry Date"+ fldSprtr;//+ fldSprtr + "Pages" 
            //lajin >> mTotalinterest then mTotalcharges
            ...
            foreach (DataRow mRow in DSstatementRaw.Tables["tStatementMasterTable"].Rows)
            {
                ...
                //  REMOVE to print sup cards if (prevAccountNo != masterRow[mAccountno].ToString())
                //{
    ...
    clsMaintainData.fixAddress(curBranchVal, masterRow[mCustomeraddress1].ToString(), out newaddress1, out newaddress2);
       ...
       //card product -contract no
         + masterRow[mCardproduct].ToString() + fldSprtr
         +masterRow[mContractno].ToString()+fldSprtr
         + masterRow[mAccountno].ToString() + fldSprtr
         + masterRow[mCardbranchpart].ToString() +
        // "  "
         //+ masterRow[mCardbranchpartname].ToString() +
         fldSprtr
         + String.Format("{0,8:dd/MM/yyyy}", masterRow[mStatementdateto]) + fldSprtr
         //+ fldSprtr + masterRow[mCardno].ToString() 
         //+ basText.formatCardNumber(curMainCard) + fldSprtr 
         //+ basText.formatCardNumber(masterRow[mCardno].ToString()) + fldSprtr
         + masterRow[mCardno].ToString() + fldSprtr
         ...
         + basText.formatDate(masterRow[mStetementduedate], "dd/MM/yyyy") + fldSprtr
         ...
         + basText.formatNum(Convert.ToDecimal(masterRow[mTotalinterest]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalinterest]))
         + fldSprtr
         + basText.formatNum(Convert.ToDecimal(masterRow[mTotalcharges]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalcharges]))
         + fldSprtr
         ...
         + extAccNum + fldSprtr
         + masterRow[mCardprimary].ToString() + fldSprtr
         + masterRow[mPrinarycardno].ToString() + fldSprtr
         //bsayed
         + rewardRow[mEarnedBonus] + fldSprtr
         + rewardRow[mRedeemedBonus] + fldSprtr +
         rewardRow[mExpiredBonus] + fldSprtr + rewardRow["CREDITCONTRACTS"].ToString() + fldSprtr + masterRow["OVERDUEDAYS"]
         + fldSprtr + basText.formatDate(masterRow[mCardexpirydate], "dd/MM/yyyy") + fldSprtr
         );
        }
        else
        {
      ...
      //  "  "
        ...
        + basText.formatNum(masterRow[mTotalpayments], "#,##0.00", 12) + DbCr(Convert.ToDecimal(masterRow[mTotalpayments])) + fldSprtr
        + basText.formatNum(Convert.ToDecimal(masterRow[mTotalpurchases]) + Convert.ToDecimal(masterRow[mTotalcashwithdrawal]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalpurchases]) + Convert.ToDecimal(masterRow[mTotalcashwithdrawal])) + fldSprtr
        + basText.formatNum(Convert.ToDecimal(masterRow[mTotalinterest]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalinterest])) + fldSprtr
        + basText.formatNum(Convert.ToDecimal(masterRow[mTotalcharges]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalcharges])) + fldSprtr
       ...
       + fldSprtr
       + fldSprtr +
        fldSprtr +  fldSprtr + masterRow["OVERDUEDAYS"]
        + fldSprtr + 
        basText.formatDate(masterRow[mCardexpirydate], "dd/MM/yyyy")
        + fldSprtr
        );
        }
    ...
    protected virtual void printDetail()
    {
        ...
        if (detailRow[dInstallmentData].ToString().Trim() != "")
            ...
            if (trnsDesc == "Installment repayment")
                ...
                //streamWritTrans.WriteLine(basText.formatCardNumber(detailRow[dCardno].ToString()) + fldSprtr + trnsDate.ToString("dd/MM/yyyy") + fldSprtr + postingDate.ToString("dd/MM/yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(basText.trimStr(input.Substring(0, input.IndexOf(',')) + input.Substring(basText.GetNthIndex(input, ',', 2)).Substring(input.Substring(basText.GetNthIndex(input, ',', 2)).IndexOf('#')).Replace('#', ' ') + " " + basText.trimStr(GetMerchantByOrigDocNo(int.Parse(detailRow[dOrigDocNo].ToString())), 40), 68), 40) + fldSprtr + strForeignCurr + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]
                streamWritTrans.WriteLine(detailRow[dCardno].ToString() + fldSprtr + trnsDate.ToString("dd/MM/yyyy") + fldSprtr + postingDate.ToString("dd/MM/yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(basText.trimStr(input.Substring(0, input.IndexOf(',')) + input.Substring(basText.GetNthIndex(input, ',', 2)).Substring(input.Substring(basText.GetNthIndex(input, ',', 2)).IndexOf('#')).Replace('#', ' ') + " " + basText.trimStr(GetMerchantByOrigDocNo(int.Parse(detailRow[dOrigDocNo].ToString())), 40), 68), 40) + fldSprtr + strForeignCurr + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]    
            }
            else
                //streamWritTrans.WriteLine(basText.formatCardNumber(detailRow[dCardno].ToString()) + fldSprtr + trnsDate.ToString("dd/MM/yyyy") + fldSprtr + postingDate.ToString("dd/MM/yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(trnsDesc.Trim(), 40) + fldSprtr + strForeignCurr + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]
                streamWritTrans.WriteLine(detailRow[dCardno].ToString() + fldSprtr + trnsDate.ToString("dd/MM/yyyy") + fldSprtr + postingDate.ToString("dd/MM/yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(trnsDesc.Trim(), 40) + fldSprtr + strForeignCurr + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]
