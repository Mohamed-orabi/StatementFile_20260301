...
// Branch X
public class clsStatRawData_VCBK : clsBasStatement
    {
    ...
    public string Statement(string pStrFileName, DataSet pDSstatement)
        ...
        try
            ...
            //strFileBasic = clsBasFile.getPathWithoutExtn(pStrFileName) + ".txt";
            ...
            string strFileHeader = "CardHolder Name" + fldSprtr + "Address1" + fldSprtr + "Address2" + fldSprtr + "Address3" +
              ...
              fldSprtr + "Purch.Cash&Dr" + fldSprtr + "Finance Charge" + fldSprtr + "External NO" + fldSprtr + "Email" + fldSprtr + "Total Due Amount" + fldSprtr;//+ fldSprtr + "Pages" 
            // VCBK-7
            //string strFileHeader = "ROWNUM" + fldSprtr + "CARDNO" + fldSprtr + "TRANSDATE" + fldSprtr + "POSTINGDATE" + fldSprtr + "CURRENCY" +
            //    fldSprtr + "DESCRIPTION" + fldSprtr + "REFERENCE" + fldSprtr + "DEBITS" + fldSprtr + "CREDITS" + fldSprtr + "CARDHOLDER NAME" +
            //    fldSprtr + "ADDRESS1" + fldSprtr + "ADDRESS2" + fldSprtr + "ADDRESS3" + fldSprtr + "ADDRESS4" + fldSprtr + "CARDTYPE" +
            //    fldSprtr + "BRANCH" + fldSprtr + "STMTDATE" + fldSprtr + "CARDNO" + fldSprtr + "CREDITLIMIT" +
            //    fldSprtr + "Available Credit" + fldSprtr + "Min. Payment Due" + fldSprtr + "New Balance" + fldSprtr + "Payment Due Date" +
            //    fldSprtr + "Past Due Amount" + fldSprtr + "prev.balance" + fldSprtr + "payment & CR" + fldSprtr + "Purch.Cash&Dr" +
            //    fldSprtr + "Interests & Charges" + fldSprtr + "External NO" + fldSprtr + "Email" + fldSprtr + "PAYMENT DUE" + fldSprtr + "CURRENT BALANCE" + fldSprtr;
            ...
            //streamWritBasic.WriteLine(strFileHeader + " Card No." + fldSprtr + "Date of Trans" + fldSprtr + "Date of Post" + fldSprtr + "Reference" + fldSprtr + "Description" + fldSprtr + "Purchase Currency & Amount" + fldSprtr + "Account Currency" + fldSprtr + "Amount" + fldSprtr);//detailRow[dBilltranamountsign]
            streamWritTrans.WriteLine("Card No." + fldSprtr + "Date of Trans" + fldSprtr + "Date of Post" + fldSprtr + "Reference" + fldSprtr + "Description" + fldSprtr + "Purchase Currency & Amount" + fldSprtr + "Account Currency" + fldSprtr + "Amount" + fldSprtr);//detailRow[dBilltranamountsign]
            streamWritBasic.WriteLine(strFileHeader);//detailRow[dBilltranamountsign]
            ...
            foreach (DataRow mRow in DSstatementRaw.Tables["tStatementMasterTable"].Rows)
                ...
                if (prevAccountNo != masterRow[mAccountno].ToString())
                    ...
                    if (emailRows.Length != 0)
                        emailaddress = emailRows[0][1].ToString().Trim();
            ...
            if (preExit)
                ...
                aryLstFiles.Add(@clsBasFile.getPathWithoutExtn(pStrFileName) + "_Summary.txt");//+ "_Raw"
    ...
    protected virtual void printHeader()
        ...
        streamWritBasic.WriteLine(masterRow[mCustomername].ToString() + fldSprtr
          ...
          + extAccNum + fldSprtr
          + emailaddress + fldSprtr
          + masterRow[mTotalDueAmount].ToString() + fldSprtr
    ...
    protected virtual void printDetail()
        ...
        streamWritTrans.WriteLine(curMainCard + fldSprtr + trnsDate.ToString("dd/MMM/yyyy") + fldSprtr + postingDate.ToString("dd/MMM/yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(trnsDesc.Trim(), 40) + fldSprtr + strForeignCurr + fldSprtr + detailRow[mAccountcurrency].ToString().Trim() + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]
        totNoOfTransactions++;
        }

    //protected virtual void print() //VCBK-7
    //    {
    //    string newaddress1, newaddress2;
    //    clsMaintainData.fixAddress(curBranchVal, masterRow[mCustomeraddress1].ToString(), out newaddress1, out newaddress2);
    //    extAccNum = clsBasValid.validateStr(masterRow[mExternalno]);
    ...
    //    //  + extAccNum + fldSprtr
    //    //  + emailaddress + fldSprtr
    //    //  + curMainCard + fldSprtr + trnsDate.ToString("dd-MMM-yyyy") + fldSprtr + postingDate.ToString("dd-MMM-yyyy") + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + basText.trimStr(trnsDesc.Trim(), 40) + fldSprtr + strForeignCurr + fldSprtr + detailRow[mAccountcurrency].ToString().Trim() + fldSprtr + basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") + CrDbDetail + fldSprtr);//detailRow[dBilltranamountsign]//clsBasValid.validateDate(detailRow[dTransdate])//clsBasValid.validateDate(detailRow[dPostingdate]) //masterRow[mCardno]
    //    //VCBK-7
    //    streamWritBasic.WriteLine(CurPageRec4Dtl + fldSprtr + curMainCard + fldSprtr + trnsDate.ToString("dd-MMM-yyyy") + fldSprtr + postingDate.ToString("dd-MMM-yyyy") + fldSprtr + detailRow[mAccountcurrency].ToString().Trim() +
    //        fldSprtr + basText.trimStr(trnsDesc.Trim(), 40) + fldSprtr + basText.trimStr(detailRow[dRefereneno], 25) + fldSprtr + (detailRow[dBilltranamountsign].ToString() == "DB" ? basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") : basText.formatNum(0, "#,##0.00;(#,##0.00)")) + fldSprtr + (detailRow[dBilltranamountsign].ToString() == "CR" ? basText.formatNum(detailRow[dBilltranamount], "#,##0.00;(#,##0.00)") : basText.formatNum(0, "#,##0.00;(#,##0.00)")) + fldSprtr + masterRow[mCustomername].ToString() +
    //        fldSprtr + newaddress1 + fldSprtr + newaddress2 + fldSprtr + masterRow[mCustomeraddress3].ToString() + fldSprtr + masterRow[mCustomerregion].ToString() + "  " + masterRow[mCustomercity].ToString() + fldSprtr + masterRow[mCardproduct] +
    //        fldSprtr + masterRow[mCardbranchpart].ToString() + "  " + masterRow[mCardbranchpartname].ToString() + fldSprtr + String.Format("{0,8:dd/MM/yy}", masterRow[mStatementdateto]) + fldSprtr + curMainCard + fldSprtr + masterRow[mAccountlim].ToString() +
    //        fldSprtr + masterRow[mAccountavailablelim].ToString() + fldSprtr + masterRow[mMindueamount].ToString() + fldSprtr + masterRow[mClosingbalance].ToString() + CrDb(Convert.ToDecimal(masterRow[mClosingbalance].ToString())) + fldSprtr + basText.formatDate(masterRow[mStetementduedate], "dd/MM/yy") +
    //        fldSprtr + masterRow[mTotaloverdueamount].ToString() + fldSprtr + basText.formatNum(masterRow[mOpeningbalance], "#,##0.00", 12) + " " + CrDb(Convert.ToDecimal(masterRow[mOpeningbalance].ToString())) + fldSprtr + basText.formatNum(masterRow[mTotalpayments], "#,##0.00", 12) + DbCr(Convert.ToDecimal(masterRow[mTotalpayments])) + fldSprtr + basText.formatNum(Convert.ToDecimal(masterRow[mTotalpurchases]) + Convert.ToDecimal(masterRow[mTotalcashwithdrawal]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalpurchases]) + Convert.ToDecimal(masterRow[mTotalcashwithdrawal])) +
    //        fldSprtr + basText.formatNum(Convert.ToDecimal(masterRow[mTotalcharges]) + Convert.ToDecimal(masterRow[mTotalinterest]), "#,##0.00", 12) + CrDbMinus(Convert.ToDecimal(masterRow[mTotalcharges]) + Convert.ToDecimal(masterRow[mTotalinterest])) + fldSprtr + extAccNum + fldSprtr + emailaddress + fldSprtr + masterRow[mTotalDueAmount].ToString() + fldSprtr + masterRow[mClosingbalance].ToString() + fldSprtr);
    //    totNoOfPageStat++;
    //    totNoOfTransactions++;
    //    }
