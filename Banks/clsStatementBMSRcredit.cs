...
// Branch 4
public class clsStatementBMSRcredit : clsBasStatement
    {
    ...
    private string cutomerPhone = string.Empty;
    private DataRow[] customerRows = null;

    public clsStatementBMSRcredit()
        {
        }

    private bool havePhone(string pClientID)
    {
        bool rtrnVal = false;
        cutomerPhone = string.Empty;
        customerRows = null;
        try
        {
            customerRows = DSemails.Tables["Emails"].Select("idclient = " + pClientID);
            for (int i = 0; i < customerRows.Length; i++)
            {
                if (!string.IsNullOrEmpty(customerRows[i][2].ToString().Trim()))
                {
                    cutomerPhone = customerRows[i][2].ToString().Trim();
                    rtrnVal = true;
                    break;
    ...
    public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pDate, string pStmntType, bool pAppendData)
        ...
        try
            ...
            //BMSR-1100
            strmRawData.WriteLine("Card Number|Customer Name|Customer Address|Contract Number|");  //masterRow[mCardno].ToString()

            // set branch for data
            curBranchVal = pBankCode; // 4; //4  = real   1 = test
            isFullFields = false;
            //mainTableCond = " m.statementno in(SELECT x.statementno FROM tstatementdetailtable x where x.branch = " + pBankCode + " GROUP BY x.statementno HAVING COUNT(*) > 4) "; //" cardproduct != 'Visa Business' ";
            //supHaving = " and d.statementno in(SELECT x.statementno FROM tstatementdetailtable x where x.branch = " + pBankCode + " GROUP BY x.statementno HAVING COUNT(*) > 4) ";
            // data retrieve
            //if (isSavedDataset)
            //{
            //  //FillStatementDataSet(pBankCode); //DSstatement =  //6); // 6
            //  clsSaveDataset loadDataset = new clsSaveDataset();
            //  //DSstatement.Clear();
            //  DSstatement = loadDataset.load(clsBasFile.getPathWithoutExtn(pStrFileName) + ".dat");
            //  //DSstatement.Reset();
            //}
            //else
            //{
            ...
            //  saveDataset.save(DSstatement, clsBasFile.getPathWithoutExtn(pStrFileName) + ".dat");
            //}
            //int VV = DSstatement.Tables["tStatementMasterTable"].Rows.Count;
            //VV = DSstatement.Tables["tStatementDetailTable"].Rows.Count;
            ...
            strOrder = " m.CARDBRANCHPART,m.accountno,m.cardprimary desc";
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
                ...
                if (prevAccountNo != masterRow[mAccountno].ToString())
                    ...
                    //BMSR-1100
                    //strmRawData.WriteLine(curMainCard + "|" + CurCustomername + " " + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + masterRow[mContractno].ToString());  //masterRow[mCardno].ToString()
                    //BMSR-7541
                    if (havePhone(masterRow[mClientid].ToString()))
                    {
                        if (cutomerPhone.Length < 10)
                        {
                            cutomerPhone = basText.replicats(" ", 14);
                        }
                        else if (cutomerPhone.Contains("(") && cutomerPhone.Contains(")"))
                        {
                            cutomerPhone = cutomerPhone.Replace("(", "").Replace(")", "");
                            if (cutomerPhone.Substring(0, 2).Equals("01")) cutomerPhone = "002" + cutomerPhone;
                            if (cutomerPhone.Substring(0, 2).Equals("20")) cutomerPhone = "00" + cutomerPhone;
                        }

                    }
                    if (masterRow[mContractno].ToString().Length == 20)
                        strmRawData.WriteLine(curMainCard + "|" + CurCustomername + " " + "|" + cutomerPhone + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + masterRow[mContractno].ToString());  //masterRow[mCardno].ToString()
                    else
                        strmRawData.WriteLine(curMainCard + "|" + CurCustomername + " " + "|" + cutomerPhone + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + basText.replicats("0", 20 - masterRow[mContractno].ToString().Length) + masterRow[mContractno].ToString());  //masterRow[mCardno].ToString()
                    
                    //BMSR-1771
                    //if (masterRow[mContractno].ToString().Length == 20)
                    //    strmRawData.WriteLine(curMainCard + "|" + CurCustomername + " " + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + masterRow[mContractno].ToString());  //masterRow[mCardno].ToString()
                    //else
                    //    strmRawData.WriteLine(curMainCard + "|" + CurCustomername + " " + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + basText.replicats("0", 20 - masterRow[mContractno].ToString().Length) + masterRow[mContractno].ToString());  //masterRow[mCardno].ToString()
            ...
            //add last page OMR
            streamWrit.WriteLine(strEndOfPage);
            streamWrit.WriteLine(clientstart);//new omr
            //if (!isSavedDataset)
            //{
            //  clsSaveDataset saveDataset = new clsSaveDataset();
            //  saveDataset.save(DSstatement, clsBasFile.getPathWithoutExtn(pStrFileName) + ".dat");
    ...
    protected void printHeader()
        ...
        streamWrit.WriteLine((basText.isContainArbic(ValidateArbic(newaddress1.Trim() + newaddress2.Trim() + masterRow[mCustomeraddress3].ToString().Trim())) ? "A" : "E") + basText.replicat(" ", 84) + "Page " + pageNo.ToString() + " of " + totalAccPages.ToString());  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
        //streamWrit.WriteLine("*" + basText.alignmentLeft(masterRow[mAccountzipcode], 5) + "*");  //old omr
        streamWrit.WriteLine("*****");  //new omr
        //originalString = ValidateArbic(masterRow[mCustomeraddress1].ToString());
        //        string[] splitArray = Regex.Split(originalString,
        ...
        //else
        //{
        //    newaddr1 = originalString;
        //    newaddr2 = string.Empty;
        //}
        ...
        //arabicStr = basText.isContainArbic(newaddr1) ? "A" : "E";
        ...
        //streamWrit.WriteLine(basText.replicat(" ", 15) + arabicStr + basText.alignmentLeft(newaddr1, 50)); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
        ...
        //arabicStr = basText.isContainArbic(newaddr2) ? "A" : "E";
        ...
        //streamWrit.WriteLine(basText.replicat(" ", 15) + arabicStr + basText.alignmentLeft(newaddr2, 50) + basText.replicat(" ", 12) + basText.formatDate(masterRow[mStatementdateto], "dd/MM/yyyy"));
        ...
        streamWrit.WriteLine(basText.replicat(" ", 15) + arabicStr + basText.alignmentLeft(ValidateArbic(masterRow[mCustomeraddress3].ToString() + " " + masterRow[mCustomerregion].ToString() + " " + masterRow[mCustomercity].ToString()), 50));  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
        ...
        streamWrit.WriteLine(basText.replicat(" ", 77) + basText.formatNum2(masterRow[mAccountlim], "########", 16, "L"));  //cardproduct+"{0,5:MM/yy}",masterRow[mStatementdatefrom]
        ...
        //streamWrit.WriteLine(String.Empty);
        //BMSR-1100
        //streamWrit.WriteLine(masterRow[mContractno].ToString());
        //BMSR-1771
        if (masterRow[mContractno].ToString().Length == 20)
            streamWrit.WriteLine(masterRow[mContractno].ToString());
        else
            streamWrit.WriteLine(basText.replicats("0", 20 - masterRow[mContractno].ToString().Length) + masterRow[mContractno].ToString());
    ...
    public void GenerateStatementLog()
        ...
        StatSummaryDetail.AccountNo = masterRow[mAccountno].ToString();
