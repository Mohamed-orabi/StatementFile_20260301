...
// Branch 16
public class clsStatTxtLblDbFBN : clsBasStatement
    {
    ...
    protected int totNoOfCardStat, totNoOfCardStatNGN, totNoOfCardStatUSD, totNoOfPageStat, totNoOfCardEmailStat, totNoOfCardEmailStatNGN, totNoOfCardEmailStatUSD, totNoOfPageEmailStat;
    ...
    protected void printStatementSummary()
        ...
        totNoOfCardStatNGN = 0;
        totNoOfCardEmailStatNGN = 0;
        totNoOfCardStatUSD = 0;
        totNoOfCardEmailStatUSD = 0;
        foreach (DataRow pRow in DSProducts.Tables["Products"].Rows)
            ...
            foreach (DataRow mRow in productRows)
                ...
                if (prevAccountNo != masterRow[accountNoName].ToString())
                    ...
                    if (totAccRows < 1 && Convert.ToDecimal(masterRow[mClosingbalance].ToString()) == 0) //          || (masterRow[mCardno].ToString() == curMainCard   // Convert.ToDecimal(
                        {
                        //if (!(Convert.ToString(curBrancchVal) == "39" && strBankName == "MBL_Debit"))
                        //{
                        continue;
                        //}
                        }
                    prevAccountNo = masterRow[accountNoName].ToString();
                    Curr = masterRow[mAccountcurrency].ToString();
                    if (Curr == "NGN")
                        totNoOfCardStatNGN++;
                    else
                        totNoOfCardStatUSD++;
            ...
            if (totNoOfCardStatNGN != 0)
                {
                StatSummary.NoOfStatements = totNoOfCardStatNGN;
                StatSummary.ProductName = "Prepaid Others NGN";
                StatSummary.ProductCode = int.Parse(ProductRow["code"].ToString());
                if (int.Parse(ProductRow["code"].ToString()) == 2832)
                    StatSummary.InsertRecordDb(vCurDate.ToString("yyyyMM") + strBankName);
                totNoOfCardStatNGN = 0;
                }
            if (totNoOfCardStatUSD != 0)
                {
                StatSummary.NoOfStatements = totNoOfCardStatUSD;
                StatSummary.ProductName = "Prepaid Others USD";
                StatSummary.ProductCode = int.Parse(ProductRow["code"].ToString());
                StatSummary.InsertRecordDb(vCurDate.ToString("yyyyMM") + strBankName);
                totNoOfCardStatUSD = 0;
                }
