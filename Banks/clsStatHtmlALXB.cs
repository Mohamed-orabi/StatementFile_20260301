...
public class clsStatHtmlALXB : clsBasStatement
{
    ...
    public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pCurDate, string pStmntType, bool pAppendData, string pReportName)
    {
        ...
        try
        {
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
            {
                ...
                emailLabelTmp = "ALEXBANK (" + vCurDate.ToString("MMMM") + ") Credit Card Statement";
                ...
                else
                {
                    ...
                    {
                        stmtpass = !string.IsNullOrWhiteSpace(strPrimaryCardNo) ? strPrimaryCardNo.Substring(strPrimaryCardNo.Length - 6) : curCardNumber.Substring(curCardNumber.Length - 6);
                    }
    ...
    protected void printBody()
    {
        ...
        emailStr.Append(@"<br /><br /><span style=""font-size: 9pt;  font-family: Arial, Helvetica, sans-serif;"">For Contract Type " + masterRow[mContracttype]?.ToString() ?? "Contract Type" + ".</span>");
