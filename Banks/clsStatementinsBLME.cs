...
// Branch 4
public class clsStatementinsBLME : clsBasStatement
{
    private string strBankName;
    private string strFileName;
    private FileStream fileStrm, fileStrm2, fileSummary;
    private StreamWriter streamWrit, streamWrit2, streamSummary;
    ...
    protected string installmentCond = "'BLOM Installment Card Program'";//'New Reward Contract'
    protected bool isInstallment = false;//'New Reward Contract'

    public clsStatementinsBLME()
    {
    }
    ...
    protected void printHeader()
    {
        ...
        streamWrit.WriteLine(String.Empty);
        streamWrit.WriteLine(String.Empty);
        streamWrit.WriteLine(basText.replicat(" ", 85) + "Page " + pageNo.ToString() + " of " + totalAccPages.ToString());  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
        streamWrit.WriteLine("*" + basText.alignmentLeft(masterRow[mAccountzipcode], 5) + "*");  //
        streamWrit.WriteLine(basText.replicat(" ", 15) + basText.alignmentLeft((object)(masterRow[mCustomertitle].ToString().Trim() + " " + masterRow[mCustomername].ToString().Trim()), 30) + basText.replicat(" ", 33) + basText.alignmentLeft(basText.formatCardNumber(curMainCard), 16));  //

        streamWrit.WriteLine(basText.replicat(" ", 15) + basText.alignmentLeft(ValidateArbic(masterRow[mCustomeraddress1].ToString()), 80));
        streamWrit.WriteLine(basText.replicat(" ", 15) + basText.alignmentLeft(ValidateArbic(masterRow[mCustomeraddress2].ToString()), 60) + basText.replicat(" ", 3) + basText.formatDate(masterRow[mStatementdateto], "dd/MM/yyyy"));
        streamWrit.WriteLine(basText.replicat(" ", 15) + basText.alignmentLeft(ValidateArbic(masterRow[mCustomeraddress3].ToString().Trim()) + " " + masterRow[mCustomerregion].ToString().Trim() + " " + masterRow[mCustomercity].ToString().Trim(), 80));
        ...
        streamWrit.WriteLine(basText.replicat(" ", 15) + basText.replicat(" ", 63) + basText.alignmentLeft(masterRow[mCardbranchpart].ToString().Trim(), 10));//String.Empty
        ...
        //streamWrit.WriteLine("áÏæÇÚì ÇáÓÑíÉ Êã ÊÔÝíÑ ÑÞã ÇáÈØÇÞÉ ÇáãØÈæÚ Ýì ßÔÝ ÇáÍÓÇÈ ¡ ÇáÑÌÇÁ ÚäÏ ÇÑÓÇá ãäÏæÈ ááÓÏÇÏ ÊÃßÏ ãä ßÊÇÈÉ ÑÞã ÇáÈØÇÞÉ ÈæÖæÍ ÍÊì íÊÓäì áäÇ ÇÖÇÝÉ ÇáãÈáÛ ÇáãÏÝæÚ Ýì ÍÓÇÈ ÇáÈØÇÞÉ ÈÕæÑÉ ÕÍíÍÉ.");//Message1
        ...
        streamWrit.WriteLine(statBoxMessage);
        ...
        //streamWrit.WriteLine(msg);//Message1
        streamWrit.WriteLine("For your security; we have masked the card number in your statement; please make sure that you typed the card number in full within your payment to guarantee proper payment process with no errors.");//Message2
        streamWrit.WriteLine(String.Empty);//Message3
        streamWrit.WriteLine(String.Empty);
        streamWrit.WriteLine(String.Empty);
        streamWrit.WriteLine(String.Empty);
    ...
    protected void printCardFooter()
    {
        ...
        streamWrit.WriteLine(basText.replicat(" ", 10) + basText.alignmentLeft((object)masterRow[mExternalno], 30)); //, "#,##0.00", 12) + " " + DbCr(Convert.ToDecimal(masterRow[mTotalpayments])), 15)
        streamWrit.WriteLine(String.Empty);
        if (isInstallment == true) // && !InstallmentCondition.Contains("BDC Easy Payment Plan"))
        {
            installmentRows = DSInstallment.Tables["installment"].Select("CLIENTID = '" + clsBasValid.validateStr(masterRow[mClientid]).ToString().Trim() + "' and CREDITCONTRACTS = '" + clsBasValid.validateStr(masterRow[mContractno]).ToString().Trim() + "'");
