...
public class clsStatHtmlALXB_CP : clsBasStatement
    {
    ...
    private string emailLabel, strBankLogo, strBackGround, backimg1, backimg2, backimg3, footLogoimage, footFacebookimage, footLinkedInimage, footYoutubeimage, footDetailimage, footOffersimage, footerimage;
    ...
    private string strbankFacebookLink, strbankLinkedInLink, strbankYouTubeLink, strbankOffersLink;
    ...
    public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pCurDate, string pStmntType, bool pAppendData, string pReportName)
        ...
        try
            ...
            if (createCorporateVal)
                ...
                // Send to the Corporate (not to the cardholder)
            ...
            //FillStatementDataSet_SortCardPriority_Exclude_VisaCards(pBankCode, ""); //DSstatement =  //10); //3
            FillStatementDataSet(pBankCode, ""); //DSstatement =  //10); //3
            getTClientBank(pBankCode);
            ...
            foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
            {
                ...
                if (prevAccountNo != masterRow[accountNoName].ToString())
                {
                    ...
                    curFileName = vCurDate.ToString("MMMM") + clsBasFile.Encrypt(strPrimaryCardNo, true);
                    ...
                    DataRow[] emailRows;
                    emailRows = DStClientBank.Tables["tClientBank"].Select("idclient = " + masterRow[ClientID].ToString());
                ...
                //emailLabelTmp = "Your ALEX Bank " + masterRow[mAccountcurrency].ToString() + " Credit Card Statement " + vCurDate.ToString("MMMM yyyy ");
                emailLabelTmp = "ALEXBANK (" + vCurDate.ToString("MMMM") + ") Corporate Card Statement";

                // issue of exporting another account number 
                //if (stmtnumber == null)
                ...
                //exportRptabp.exportAttachment(pStrFileName, pBankCode, curFileName, pReportName, "(IsNull({@f_POSTINGDATE}) and IsNull({@f_DOCNO}) and isnull({@f_BranchTrans}) and {@F_branch} = " + pBankCode + " and {@f_closingbalance} <> 0 and {TSTATEMENTMASTERTABLE.CARDLIMIT} <> 0  and {TSTATEMENTMASTERTABLE.ACCOUNTNO} = '" + stmtnumber + "') or (not IsNull({@f_POSTINGDATE}) and not IsNull({@f_DOCNO}) and not isnull({@f_BranchTrans}) and {@F_branch} = " + pBankCode + "  and {TSTATEMENTMASTERTABLE.CARDLIMIT} <> 0  and {TSTATEMENTMASTERTABLE.ACCOUNTNO} = '" + stmtnumber + "')");

                //Orginal hash as per Jira ALXB-3622
                ...
                {
                    // Jira (exclude all visa cards from the E-Statement) ALXB-3622
                    ...
                    exportRptabp.exportAttachment(pStrFileName, pBankCode, curFileName, pReportName,
                        ...
                        "(IsNull({@f_POSTINGDATE}) and IsNull({@f_DOCNO}) and isnull({@f_BranchTrans}) and {@F_branch} = " + pBankCode + " and {@f_closingbalance} <> 0 and {TSTATEMENTMASTERTABLE.cardaccountno} = '" + stmtnumber + "') or (not IsNull({@f_POSTINGDATE}) and not IsNull({@f_DOCNO}) and not isnull({@f_BranchTrans}) and {@F_branch} = " + pBankCode + "  and {TSTATEMENTMASTERTABLE.cardaccountno} = '" + stmtnumber + "')"
                        );
                    exportRptabp = null;
                    stmtnumber = null;
    ...
    protected void printBody()
        ...
        emailStr.Append(@"<img  Style=""width: 100%;height: auto;"" border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankbackimg1) + @"'/>");
        // emailStr.Append(@"<tr><td>");
        emailStr.Append(@"<img Style=""width: 100%;height: auto;"" border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankbackimg2) + "'/>");
        // emailStr.Append(@"<tr><td>");
        emailStr.Append(@"<img Style=""width: 100%;height: auto;"" src='cid:" + clsBasFile.getFileFromPath(bankbackimg3) + @"'/></tr></td><br/><br/><br/>");

        emailStr.Append(@"<tr><td>");
        emailStr.Append(@"<a href=" + strbankWebLink + @">");
        emailStr.Append(@"<img src='cid:" + clsBasFile.getFileFromPath(bankFooterLogoimg) + @"'/></a><br/>");
        emailStr.Append(@"<a href=" + strbankFacebookLink + @">");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterFacebookimg) + @"'/></a><br/>");
        emailStr.Append(@"<a href=" + strbankLinkedInLink + @">");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterLinkedInimg) + @"'/></a><br/>");
        emailStr.Append(@"<a href=" + strbankYouTubeLink + @">");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterYoutubeimg) + @"'/></a><br/>");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterDetailsimg) + @"'/><br/>");
        emailStr.Append(@"<a href=" + strbankOffersLink + @">");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterOffersimg) + @"'/></a><br/>");
        emailStr.Append(@"<img border=""0"" src='cid:" + clsBasFile.getFileFromPath(bankfooterimg) + @"'/></tr></td>");
    ...
    private void SendEmail(string pBody, string pSubject, string pTo)
        ...
        if (valdEmail.isValideEmail(pTo) != "ValidEmail")//!basText.isValideEmail(pTo)
            ...
            pLstAttachedPic.Add(bankbackimg1);
            pLstAttachedPic.Add(bankbackimg2);
            pLstAttachedPic.Add(bankbackimg3);

            pLstAttachedPic.Add(bankFooterLogoimg);
            pLstAttachedPic.Add(bankfooterFacebookimg);
            pLstAttachedPic.Add(bankfooterLinkedInimg);
            pLstAttachedPic.Add(bankfooterYoutubeimg);
            pLstAttachedPic.Add(bankfooterDetailsimg);
            pLstAttachedPic.Add(bankfooterOffersimg);
            pLstAttachedPic.Add(bankfooterimg);
            ...
            {
            //clsBasErrors.catchError(ex);
                streamNoEmails.WriteLine(curCustomerName + "|" + curAccountNumber + "|" + curClientID + "|" + " " + "|" + "| Email " + pTo + " , Err Message >> " + ex.Message );
            noOfBadEmails++;
            }
        finally
        {
    ...
    public string bankLinkedInLink
        {
        get { return strbankLinkedInLink; }
        set { strbankLinkedInLink = value; }
        }
    ...
    public string bankoffersLink
        {
        get { return strbankOffersLink; }
        set { strbankOffersLink = value; }
        }
    ...
    //iatta add statement images 
    public string bankbackimg1
        {
        get { return backimg1; }
        set { backimg1 = value; }
        }
    public string bankbackimg2
        {
        get { return backimg2; }
        set { backimg2 = value; }
        }
    public string bankbackimg3
        {
        get { return backimg3; }
        set { backimg3 = value; }
        }

    public string bankFooterLogoimg
        {
        get { return footLogoimage; }
        set { footLogoimage = value; }
        }
    public string bankfooterFacebookimg
        {
        get { return footFacebookimage; }
        set { footFacebookimage = value; }
        }
    public string bankfooterLinkedInimg
        {
        get { return footLinkedInimage; }
        set { footLinkedInimage = value; }
        }
    public string bankfooterYoutubeimg
        {
        get { return footYoutubeimage; }
        set { footYoutubeimage = value; }
        }
    public string bankfooterDetailsimg
        {
        get { return footDetailimage; }
        set { footDetailimage = value; }
        }
    public string bankfooterOffersimg
        {
        get { return footOffersimage; }
        set { footOffersimage = value; }
        }
    public string bankfooterimg
        {
        get { return footerimage; }
        set { footerimage = value; }
        }
