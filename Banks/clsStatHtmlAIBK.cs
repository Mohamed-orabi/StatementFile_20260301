...
public class clsStatHtmlAIBK : clsBasStatement
{
    ...
    public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pCurDate, string pStmntType, bool pAppendData, string pReportName)
    {
        ...
        try
        {
            ...
            if (IsSplitted)
            {
                MainTableCond = " m.cardproduct not in '" + strProductCond +"'";//strWhereCond
                supTableCond = " d.statementno in (select x.statementno from " + clsSessionValues.mainDbSchema + clsSessionValues.mainTable + " x where x.branch = " + pBankCode + " and x.cardproduct not in " + "'" + strProductCond +"'"+ ")";
            }
