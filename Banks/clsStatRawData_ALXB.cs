...
// Branch X
public class clsStatRawData_ALXB : clsBasStatement
    {
    ...
    //if error happen >> int to long 
    private long installdocno = 0;
    private long originstalldocno = 0;
    private string installdata = string.Empty;

    public clsStatRawData_ALXB()
        {
        }
    ...
    public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pDate, string pStmntType, bool pAppendData)
       ...
       maintainData.matchCardBranch4Account(pBankCode);
        ...
        //MainTableCond = " m.contracttype in " + strProductCond + " AND m.accountno in ('4031850000041148','VS_CR_EGP_0000002450','VS_CR_EGP_0000007824') ";//strWhereCond
        //supTableCond = " d.statementno in (select x.statementno from " + clsSessionValues.mainDbSchema + clsSessionValues.mainTable + " x where x.branch = " + pBankCode + " and x.contracttype in " + strProductCond + " AND x.accountno in ('4031850000041148','VS_CR_EGP_0000002450','VS_CR_EGP_0000007824') )";
        //FillStatementDataSet_Exclude_VisaCards(pBankCode, ""); //DSstatement =  //6); // 6
    ...
    public string Statement(string pStrFileName, DataSet pDSstatement)
        ...
        try
            ...
            //lajin >> Card number, Card Status, Card State,ccbalance, External amount no ");
            //streamWritDelivery.WriteLine("STATEMENT NUMBER,Ordering Branch,CUSTOMER NAME,Cardholder Address Line #1,Cardholder Address Line #2,PHONE NUMBER,CITIZEN ID, Card number,MBR, Card Status, Card State, Card expiry date,  External No, Opening balance, Closing balance, Total Due Amount, Total overdue amount, Min due amount, Overdue days,CREDIT CONTRACTS ");
    ...
    protected virtual void printDelivery()
        ...
        //fldSprtr is , 
        streamWritDelivery.WriteLine(basText.alignmentLeft(masterRow[mStatementnumber], 11).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mCardbranchpartname].ToString(), 15).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mCustomername].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(ValidateArbic(newaddress1), 50).Trim() + fldSprtr
            + basText.alignmentLeft(ValidateArbic(newaddress2), 50).Trim() + fldSprtr
            + basText.alignmentLeft(CustomerPhone, 50).Trim() + fldSprtr
            + basText.alignmentLeft(CitizenID.ToString(), 15).Trim()
            //lajin Card number, Card Status, Card State,ccbalance, External amount no ");
            /*
            + basText.alignmentLeft(masterRow[mCardno].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mMBR].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mCardstatus].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mCardstate].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mCardexpirydate].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mExternalno].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mOpeningbalance].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mClosingbalance].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mTotalDueAmount].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mTotaloverdueamount].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mMindueamount].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mOverduedays].ToString(), 100).Trim() + fldSprtr
            + basText.alignmentLeft(masterRow[mcontractstate].ToString(), 100).Trim()
            */
            );
