...
public class clsStatHtmlIMB : clsBasStatement
    {
    ...
    protected void printHeader()
        ...
        emailStr.Append(MakeHeaderStr(basText.formatNum(masterRow[mTotaloverdueamount], "##0", 13)));
        emailStr.Append(@"</td></tr>");
    ...
    protected void printDetail()
        ...
        if (masterRow[mAccountcurrency].ToString().Trim() != detailRow[dOrigtrancurrency].ToString().Trim())
            strForeignCurr = basText.formatNumUnSign(detailRow[dOrigtranamount], "##,###,##0;(#,###,##0)", 13) + " " + basText.Replace(clsBasValid.validateStr(detailRow[dOrigtrancurrency]), "XXX", "   ");
        else
        ...
        emailStr.Append(MakeHeaderStr((basText.formatNumUnSign(detailRow[dBilltranamount], "#,###,##0", 14) + " " + CrDb(clsBasValid.validateStr(detailRow[dBilltranamountsign])).ToString())));
        emailStr.Append(@"</td>");
    ...
    protected void printCardFooter()
        ...
        emailStr.Append(@"<tr><td width=""99%"" colSpan=""6"" height=""21"">For more information , please call 3227 or email us at cardsenquiry@imbank.co.rw</td></tr>");
        //emailStr.Append(@"</td></tr>");
        //emailStr.Append(@"<tr><td>");
        ...
        emailStr.Append(MakeHeaderStr(basText.alignmentRight(basText.formatNumUnSign(masterRow[mOpeningbalance], "#,###,##0", 12) + " " + CrDb(Convert.ToDecimal(masterRow[mOpeningbalance].ToString())), 15)));
        emailStr.Append(@"</td>");
        emailStr.Append(@"<td align='center'>");
        emailStr.Append(MakeHeaderStr(basText.alignmentRight(basText.formatNumUnSign(masterRow[mTotalpayments], "#,###,##0", 12) + " " + DbCr(Convert.ToDecimal(masterRow[mTotalpayments])), 15)));
        emailStr.Append(@"</td>");
        ...
        emailStr.Append(MakeHeaderStr(basText.alignmentRight(basText.formatNumUnSign(masterRow[mTotalinterest].ToString(), "#,###,##0", 12) + " " + CrDbMinus(Convert.ToDecimal(masterRow[mTotalinterest])), 15)));
        emailStr.Append(@"</td>");
        emailStr.Append(@"<td align='center' width='70'>");
        emailStr.Append(MakeHeaderStr(basText.alignmentRight(basText.formatNumUnSign(masterRow[mTotalcharges].ToString(), "#,###,##0", 12) + " " + CrDbMinus(Convert.ToDecimal(masterRow[mTotalcharges])), 15)));
        emailStr.Append(@"</td>");
        emailStr.Append(@"<td width='97' align='center'>");
        emailStr.Append(@"<font size='1' color='#800000'>");
        emailStr.Append(MakeHeaderStr(basText.alignmentRight(basText.formatNumUnSign(masterRow[mClosingbalance], "#,###,##0", 16) + " " + CrDb(Convert.ToDecimal(masterRow[mClosingbalance])), 15)));
        emailStr.Append(@"</font></td>");

        emailStr.Append(@"<td align='center' width='85'>");
        emailStr.Append(MakeHeaderStr(basText.formatNum(masterRow[mTotaldebits], "#0;(#0)")));
        emailStr.Append(@"</td>");
        emailStr.Append(@"<td align='center' width='82'>");
        emailStr.Append(MakeHeaderStr(basText.formatNum(masterRow[mTotalcredits], "#0;(#0)")));
        emailStr.Append(@"</td>");
        emailStr.Append(@"</tr>");
        ...
        if (isRewardVal)
            ...
            if (rewardRows.Length > 0)
                ...
                emailStr.Append(@"<tr><td align='middle' width='171'>");
                emailStr.Append(MakeHeaderStr(basText.formatNum(rewardRow[mOpeningbalance], "#0;(#0)", 20, "M"), false));
                emailStr.Append(@"</td><td align='middle'>");
                emailStr.Append(MakeHeaderStr("0", false));
                emailStr.Append(@"</td><td align='middle' width='122'>");
                emailStr.Append(MakeHeaderStr(basText.formatNum(rewardRow["EARNEDBONUS"], "#0;(#0)", 20, "M"), false));
                emailStr.Append(@"</td><td align='middle' width='121'>");
                emailStr.Append(MakeHeaderStr(basText.formatNum(rewardRow["REDEEMEDBONUS"], "#0;(#0)", 20, "M"), false));
                emailStr.Append(@"</td><td align='middle' width='136'>");
                emailStr.Append(MakeHeaderStr(basText.formatNum(rewardRow[mClosingbalance], "#0;(#0)", 20, "M"), false));
        ...
        emailStr.Append(@"<tr><td width='98%' colSpan='6'>&nbsp;</td></tr>");
        //emailStr.Append(@"<tr><td width='98%' colSpan='6' Style="" text-align: justify;""><font size=""1"" color=""Black""><B>Disclaimer: - This message is not being transmitted by secure means and its attachments / annexures / enclosures (if any), and the contents thereof, are legally privileged and confidential, for the use of the intended recipient(s) only. They should not be read, copied, retained, or used, in any manner, by anyone other than the intended recipient(s) and for the intended purpose alone. If you have received this document and the attachment(s)/annexure(s)/enclosure(s) in error or by accident, please immediately notify and return to us at the above co-ordinates, preserving their confidentiality, and not retaining the same in any manner. Neither the sender nor any I&M Bank (Rwanda) Ltd entity shall have any responsibility or liability for unauthorized access and/or alteration to this communication, nor for any consequence based on or arising from use of information that may have been accessed or altered by any person. Although I&M Bank (Rwanda) Ltd  takes all reasonable precautions to ensure that this message and any file transmitted with it is virus free, It accepts no liability for any damage that may be caused by any virus transmitted by this email. Thank you</B></font></td></tr>");
        emailStr.Append(@"<tr><td width='98%' colSpan='6'>&nbsp;</td></tr>");
        emailStr.Append(@"<tr><td width='98%' colSpan='6'>&nbsp;</td></tr>");
    ...
    protected void printAccountFooter()
        {
        streamWrit.WriteLine("GRAND" + basText.formatNumUnSign(masterRow[mOpeningbalance], "##0", 16) + basText.formatNum(masterRow[mTotaldebits], "#0;(#0)", 16) + basText.formatNum(masterRow[mTotalcredits], "#0;(#0)", 16) + basText.formatNum(masterRow[mClosingbalance], "#0;(#0)", 16) + basText.formatNum(masterRow[mMindueamount], "#0.00;(#0.00)", 16) + basText.replicat(" ", 12) + basText.formatDate(masterRow[mStetementduedate], "yyyy/MM/dd") + "\n");//+strEndOfPage
        }
...
//            //streamEmails.WriteLine(curAccountNumber + "|" + curClientID + "|" + emailCc + "|" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"));
            ...
            {
            //clsBasErrors.catchError(ex);
                streamNoEmails.WriteLine(curAccountNumber + "|" + curClientID + "|" + emailTo + "|" + (emailRow?["mobilephone"]?.ToString() ?? "no mobile number!") + "| Email " + pTo + " , Err Message >> " + ex.Message + ", Err Desc >> ");
            noOfBadEmails++;
            }
        finally
            {
