...
// Branch 1
public class clsStatTxtCpnyLbl : clsBasStatement
{
  protected string strBankName;
  protected FileStream fileStrm, fileSummary, fileRawData;
  protected StreamWriter streamWrit, streamSummary, strmRawData;
  ...
  protected const int MaxDetailInPageComp = 25; //
  protected const int MaxDetailInLastPage = 27; //
  protected int CurPageRec4Dtl = 0;
  protected int pageNo = 0, totalPages = 0, totalCardPages = 0, curAccCompanyRows = 0
  ...
  protected DataRow[] cardsRows, accountRows, dtlRows;
  protected DataRow[] mainRows;
  protected string CurrentPageFlag;
  protected string strCardNo, strPrimaryCardNo, strCurrency, strStatementNo;
  protected string strForeignCurr;
  protected string stmNo;
  protected int totNoOfCardStatCrdHldr, totNoOfPageStatCrdHldr;
  protected int totNoOfCardStatCmpny, totNoOfPageStatCmpny;
  protected string strFileName;
  protected decimal valOutstanding = 0, valCompOutstanding = 0;
  protected decimal valTransaction = 0, valCompTransaction = 0, valCompMinDueAmount = 0, compTotalOverdueAmount = 0;
  protected string curContractNo;
  ...
  protected string curMainCard, CurCardAddres1, CurCardAddres2, CurCardAddres3, CurCardNo;
  protected int totCrdNoInAcc, curCrdNoInAcc;
  protected string strOutputPath, strFileCompany, strFileCardHolder
    , fileSummaryName;
  protected DateTime vCurDate;
  protected ArrayList aryLstFiles = new ArrayList();
  protected string curContarctCurr;
  protected frmStatementFile frmMain;
  protected int totContact;


  public clsStatTxtCpnyLbl()
  {
  }
  ...
  protected void printCompanyStatement()
  {
    ...
    try
    {
      ...
      foreach (DataRow currencyRow in DSstatement.Tables["currencyTable"].Rows)
      {
        if (curContarctCurr != currencyRow[mAccountcurrency].ToString().Trim())
        {
          ...
          curContarctCurr = currencyRow[mAccountcurrency].ToString().Trim();
          ...
          fileStrm = new FileStream(strFileCompany, FileMode.Create); //FileMode.Append FileMode.Create Create
          streamWrit = new StreamWriter(fileStrm, Encoding.Default);
        }
        setContractTable(curBranchVal, curContarctCurr);
        totContact = 1;
        frmMain.BeginInvoke(frmMain.setMinMaxProgressDelegate, new object[] { DSstatement.Tables["contractTable"].Rows.Count });
    foreach (DataRow contractRow in DSstatement.Tables["contractTable"].Rows)
      {
        ...
        cardsRows = DSstatement.Tables["tStatementMasterTable"].Select("contractno = '" + curContractNo + "' and accountcurrency = '" + curContarctCurr + "' and accountcurrency = '" + curContarctCurr + "'"); //accountno
      ...
      printStatementSummary(curContarctCurr);
    }//end of Currency foreach
  }//end of try
    catch (OracleException ex)
    {
      clsDbOracleLayer.catchError(ex);
  ...
  protected virtual void printHeaderCompany()
  {
    ...
    streamWrit.WriteLine(basText.replicat(" ", 83) + "Product:");  //+"{0,5:MM/yy}",masterRow[mStatementdatefrom]
    streamWrit.WriteLine(basText.alignmentMiddle("Customer Name & Address", 80) + basText.replicat(" ", 3) + masterRow[mCardproduct]);  //+"{0,5:MM/yy}",masterRow[mStatementdatefrom]
    streamWrit.WriteLine(basText.alignmentMiddle(masterRow[mCustomername], 80) + basText.replicat(" ", 3) + "Branch:");  //
    streamWrit.WriteLine(basText.replicat(" ", 83) + masterRow[mCardbranchpartname]);  //
    streamWrit.WriteLine(basText.alignmentMiddle(masterRow[mCustomeraddress1], 80) + "  Statement Date:"); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
    ...
    streamWrit.WriteLine(basText.replicat(" ", 83) + "{0,10:dd/MM/yyyy}", masterRow[mStatementdateto]);//Statementdatefrom+ basText.formatNum(masterRow[mAccountlim],"########")  basText.formatNum(masterRow[mCardlimit],"########")
    streamWrit.WriteLine(basText.alignmentMiddle(masterRow[mCustomeraddress2], 80));
    streamWrit.WriteLine(basText.replicat(" ", 83) + "Page " + pageNo + " / " + totalAccPages);  //basText.formatNum(masterRow[mAccountno],"##-##-######-####")
    ...
    //streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(basText.alignmentMiddle("Contract No", 20) + basText.alignmentMiddle("Contract limit", 20) + basText.alignmentMiddle("Available Limit", 20) + basText.alignmentMiddle("Due Date", 20) + basText.alignmentMiddle("Total Overdue", 20)); //masterRow[mTotaloverdueamount]// + basText.formatNum(masterRow[mTotaloverdueamount],"##0.00",13)    //+ basText.formatNum(masterRow[mAccountlim],"#,##0.00",9)+" "+basText.formatNum(masterRow[mTotalcredits],"#,##0.00",9)+" "+basText.formatNum(masterRow[mAccountavailablelim],"#,##0.00",9)+" "+basText.formatNum(masterRow[mMindueamount],"#,##0.00",15)+" "+basText.formatNum(masterRow[mClosingbalance],"#,##0.00",15)+" "+basText.formatDate(masterRow[mStetementduedate],"dd/MM/yy") //TOTALCREDITS// basText.alignmentMiddle(masterRow[mAccountavailablelim],20) //"#,##0"  +  basText.alignmentMiddle(valCompMinDueAmount,13)
    streamWrit.WriteLine(basText.alignmentMiddle(masterRow[mContractno], 20) + basText.alignmentMiddle(masterRow[mContractlimit], 20) + basText.formatNum(masterRow[mAccountavailablelim], "##0", 20,"M") + basText.formatDate(masterRow[mStetementduedate], "dd/MM/yyyy", 20, "M") + basText.formatNum(compTotalOverdueAmount, "#,##0.00", 20,"M")); //masterRow[mTotaloverdueamount]// + basText.formatNum(masterRow[mTotaloverdueamount],"##0.00",13)    //+ basText.formatNum(masterRow[mAccountlim],"#,##0.00",9)+" "+basText.formatNum(masterRow[mTotalcredits],"#,##0.00",9)+" "+basText.formatNum(masterRow[mAccountavailablelim],"#,##0.00",9)+" "+basText.formatNum(masterRow[mMindueamount],"#,##0.00",15)+" "+basText.formatNum(masterRow[mClosingbalance],"#,##0.00",15)+" "+basText.formatDate(masterRow[mStetementduedate],"dd/MM/yy") //TOTALCREDITS// basText.alignmentMiddle(masterRow[mAccountavailablelim],20) //"#,##0"  +  basText.alignmentMiddle(valCompMinDueAmount,13)
    streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(String.Empty);
    //    if(pageNo == 1)
  ...
  protected virtual void printDetailCompany()
  {
  ...
  protected virtual void printCardFooterCompany()
  {
    streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(String.Empty);
    //streamWrit.WriteLine(String.Empty);
    streamWrit.WriteLine(basText.alignmentMiddle("Contract No", 20) + basText.alignmentMiddle("Total Transaction", 20) + basText.alignmentMiddle("Total MinDue Amount", 20) + basText.alignmentMiddle("Due Date", 15));
  ...
  protected void calcCardlRowsCompany()
  {
    totalCardPages = 0;
    totCardRows = 0;
  ...
  protected void calcAccountRowsCompany()
  {
    curAccRows = 0;
    accountRows = null;
    //stmNo = masterRow[mStatementno].ToString();
    ...
    accountRows = DSstatement.Tables["tStatementMasterTable"].Select("contractno = '" + curContractNo + "' and accountcurrency = '" + curContarctCurr + "' and accountcurrency = '" + curContarctCurr + "'"); //accountno
    totalAccPages = 0;
    totAccRows = 0;
  ...
  protected void completePageDetailRecordsCompany()
  {
  ...
  protected void calcAccountRowsCardHolderCompany()
  {
    curAccCompanyRows = 0;
    accountRows = null;
    //stmNo = masterRow[mStatementno].ToString();
    ...
    mainRows = DSstatement.Tables["tStatementMasterTable"].Select("cardaccountno = '" + strStatementNo + "' and accountcurrency = '" + curContarctCurr + "'");
  ...
  protected void printStatementSummary(string pCurrency)
  {
    streamSummary.WriteLine(strBankName + " Company Statement - " + pCurrency);
    ...
    totNoOfCardStatCmpny = totNoOfPageStatCmpny = 0;
    //streamSummary.WriteLine("");
  ...
  protected void add2FileList(string pFileName)
  {
