...
// Branch 16
public class clsStatTxtLblDb : clsBasStatement
    {
    ...
    protected bool AddressNotPrinted = false;
    ...
    protected virtual void printHeader()
        ...
        if (AddressNotPrinted)
            strmWriteCommon.WriteLine(basText.alignmentMiddle("", 50)); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
        else
        ...
        if (AddressNotPrinted)
            strmWriteCommon.WriteLine(basText.alignmentMiddle("", 50));
        else
        ...
        if (AddressNotPrinted)
            strmWriteCommon.WriteLine(basText.alignmentMiddle("", 50));  //basText.formatNum(masterRow[accountNoName],"##-##-######-####")
        else
    ...
    protected void printStatementSummary()
        ...
        DataRow[] productRows;
        streamSummary.WriteLine(strBankName + " Statements");
        if (isInEmailService)
            {
            ...
            if (pBranch == 12)
                ...
                foreach (DataRow pRow in DSProducts.Tables["Products"].Rows)
                    ...
                    if ((totNoOfCardStat + totNoOfCardEmailStat) != 0)
                        ...
                        StatSummary.NoOfTransactionsInt = 0;// totNoOfTransactionsInt + totNoOfTransEmailInt;
    ...
    public bool NoPrintAddress
        {
        get { return AddressNotPrinted; }
        set { AddressNotPrinted = value; }
        }// NoPrintAddress
