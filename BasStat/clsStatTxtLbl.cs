...
public class clsStatTxtLbl : clsStatTxt
{
    public clsStatTxtLbl()
    {
    }
    ...
    protected override void printHeader()
    {
        ...
        if (pBranch == 122)
            strmWriteCommon.WriteLine(basText.alignmentMiddle(masterRow[mCardtitle], 50));  //
        else
        ...
        if (pBranch == 130)
        {
            strmWriteCommon.WriteLine(basText.replicat(" ", 55) + basText.alignmentRight("Total Balance :", 25) + " " + basText.formatNumOrabi(Summary[0]["CLOSINGBALANCE"].ToString(), "##0.00"));
            strmWriteCommon.WriteLine(basText.replicat(" ", 55) + basText.alignmentRight("Total limit :", 25) + " " + Summary[0]["CARDLIMIT"].ToString());
            strmWriteCommon.WriteLine(basText.replicat(" ", 55) + basText.alignmentRight("Total Minim Due :", 25) + " " + Summary[0]["TOTALOVERDUEAMOUNT"].ToString());
            strmWriteCommon.WriteLine(String.Empty);
        }
        ...
        if (bankName == "BICV_Credit_Text" || bankName == "BCA_Credit_Text")
    ...
    protected override void printCardFooter()
    {
        ...
        {
            if (bankName == "Suez_Corporate_txt")
            {
                strmWriteCommon.WriteLine(String.Empty);
                strmWriteCommon.WriteLine(String.Empty);
                strmWriteCommon.WriteLine(String.Empty);
        ...
        if (bankName == "BICV_Credit_Text" || bankName == "BCA_Credit_Text")
        {
          ...
          + basText.alignmentMiddle("DAF Amount", 19));
            strmWriteCommon.WriteLine(basText.alignmentMiddle((curMainCard), 35) + basText.alignmentMiddle(masterRow[mMindueamount], 20) + basText.formatDate(masterRow[mStetementduedate], "dd/MM/yyyy", 15, "M")
             ...
             + basText.alignmentMiddle(basText.formatNumUnSign(masterRow[mCarddafamount].ToString() == "" ? 0 : masterRow[mCarddafamount], "#0.00", 16) + " " + CrDb(Convert.ToDecimal(masterRow[mCarddafamount].ToString() == "" ? 0 : masterRow[mCarddafamount])), 19));
        }
        else
        {
            strmWriteCommon.WriteLine(
