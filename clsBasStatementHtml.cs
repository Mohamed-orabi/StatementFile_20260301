...
//Branch X
public class clsBasStatementHtml : clsBasStatement
{
  ...
  private int MaxDetailInPage = 20; //
  ...
  private string rewardCond = "'Reward Program'";//Reward Contract//'New Reward Contract''Reward Contract'
  ...
  public string Statement(string pStrFileName, string pBankName, int pBankCode, string pStrFile, DateTime pCurDate, string pStmntType, bool pAppendData)
  {
    ...
    try
    {
      ...
      strSubTableCond = "d.trandescription !='Calculated Points' ";//'Redeem with Credit'
      ...
      foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
      {
        ...
        if (prevAccountNo != masterRow[accountNoName].ToString())
        {
          ...
          if (createCorporateVal)
          {
            emailRows = DStClientBank.Tables["tClientBank"].Select("idclient = '" + masterRow[mClientid].ToString() + "'");
          }
          else
          {
          ...
          if (emailRows.Length > 0)
          {
            if (createCorporateVal)
            {
              strmRawData.WriteLine(extAccNum + "|" + CurCustomername + " " + "|" + CurCardAddres1 + " " + CurCardAddres2 + " " + CurCardAddres3 + "|" + emailRow["phonelegal"].ToString() + "|" + emailRow["phonecont"].ToString() + "|");  //masterRow[mCardno].ToString()
              emailRow.Delete();
              //DStClientBank.Tables["tClientBank"].Rows[0].Delete .("idclient = '" + masterRow[mClientid].ToString() + "'")
            }
            else
            {
          ...
          {
            //strmRawData.WriteLine(string.Empty);  
          }
    ...
    finally
    {
      if (preExit)
      {
        ...
        if (pStmntType == "Corporate")
        {
          ...
          clsStatementBNP_CorpCmpny stmntBNPCorp = new clsStatementBNP_CorpCmpny();// + "NSGB_Business_Statement.txt"
          stmntBNPCorp.setFrm = frmMain;
          stmntBNPCorp.rewardCond = rewardCond;
  ...
  protected virtual void printHeader()
  {
    ...
    //			streamWrit.WriteLine(basText.alignmentMiddle(masterRow[Address1Name],50)+ basText.replicat(" ",31) + clsBasValid.validateStr(masterRow[accountNoName])); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
    //string x = Convert.ToString(ValidateArbic(masterRow[Address1Name].ToString()));
    streamWrit.WriteLine(basText.alignmentMiddle(ValidateArbic(masterRow[Address1Name].ToString()), 50)); //basText.formatNum(masterRow[mCardno],"####-####-####-####")
    ...
    //			streamWrit.WriteLine(basText.alignmentMiddle(masterRow[Address2Name],50)+ basText.replicat(" ",31)+"{0,10:dd/MM/yyyy}",masterRow[mStatementdatefrom]);//+ basText.formatNum(masterRow[accountLimit],"########")  basText.formatNum(masterRow[mCardlimit],"########")
    streamWrit.WriteLine(basText.alignmentMiddle(ValidateArbic(masterRow[Address2Name].ToString()), 50));
    ...
    //			streamWrit.WriteLine(basText.alignmentMiddle(masterRow[Address3Name].ToString().Trim() + " " + masterRow[mCustomerregion].ToString().Trim() + " " + masterRow[mCustomercity].ToString().Trim(),50)+ basText.replicat(" ",31)+pageNo + " / " + totalAccPages);  //basText.formatNum(masterRow[accountNoName],"##-##-######-####")
    streamWrit.WriteLine(basText.alignmentMiddle(ValidateArbic(masterRow[Address3Name].ToString().Trim()) + " " + masterRow[mCustomerregion].ToString().Trim() + " " + masterRow[mCustomercity].ToString().Trim(), 50));  //basText.formatNum(masterRow[accountNoName],"##-##-######-####")
  ...
  protected virtual void printCardFooter()
  {
    ...
    if (rewardRows.Length > 0)
    {
      ...
      streamWrit.WriteLine(basText.formatNum(rewardRow[mOpeningbalance], "#0.00;(#0.00)", 20, "L") + basText.formatNum(rewardRow["EARNEDBONUS"], "#0.00;(#0.00)", 20, "L") + basText.formatNum(rewardRow["REDEEMEDBONUS"], "#0.00;(#0.00)", 20, "L") + basText.formatNum((Convert.ToDecimal(rewardRow["bonusadjustment"]) * -1), "#0.00", 20, "L") + basText.formatNum(rewardRow[mClosingbalance], "#0.00;(#0.00)", 20, "L")); // + basText.formatNum((Convert.ToDecimal(rewardRow["bonusadjustment"]) * -1), "#0.00", 20, "L")  //streamWrit.WriteLine(String.Empty);
    }
    else
      streamWrit.WriteLine(basText.replicats(" ", 100));
  ...
  private void writeStatement(DataSet pDS, string pFileName)
  {
    ...
    try
    {
      ...
      foreach (DataRow mRow in DSstatement.Tables["tStatementMasterTable"].Rows)
      {
        ...
        foreach (DataColumn mCol in DSstatement.Tables["tStatementMasterTable"].Columns)
        ...
        {
          tmpLine = String.Empty;
          foreach (DataColumn dCol in DSstatement.Tables["tStatementDetailTable"].Columns)
  ...
  private void printStatementSummary()
  {
    ...
    clsBasStatementSummary StatSummary = new clsBasStatementSummary();
