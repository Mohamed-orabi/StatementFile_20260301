drop procedure _MhStmtTbl;

Create DBA Procedure "cap"._MhStmtTbl(pProduct Char(20), pBatchSerno Integer );
...
-- Trace for Procedure _MhStmtTbl
...
On Exception                                 -- In case of error catch it here and log it.
	...
	Trace '	Procedure: _MhStmtTbl';
	End
	Raise Exception Sql_e, Isam_e, Txt_e;
End Exception;
-- End Trace for Procedure _MhStmtTbl
...
If fCardProduct in (select name from products 
  ...
  Foreach
   ...
   -- CRAG  --fCardProduct
    ...
    insert into stmtfile values	( 'M',fnumber, fCreditLimit, fStmtAddress1
...
If fCardProduct in (select name from products 
   ...
   Let fStmtAddress5 = cap_kth_string(fAddress1, fAddress2, fAddress3, fAddress4, fAddress5, Trim(fZip) || ' ' || fCity, Null, Null, 5);
    ...
    let fi031_ARN = (select I031_arn from cisotrxns 
     ...
     insert into stmtfile values ( 'D',fnumber, fCardCreditLimit, fStmtAddress1
      , fStmtAddress2, fStmtAddress3, fStmtAddress4, fStmtAddress5
...
grant execute on procedure _MhStmtTbl to public as cap;