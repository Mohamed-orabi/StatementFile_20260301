  ...
  Begin
    ...
    begin
      select
        ...
        a.Acct_Stat, a.createdate,pl.external
      into vCompany,vNameComp,vAddressCont,vCountryCont,vRegionCont,vCityCont,
        ...
        vAccStat,vAccountCreateDate,vExternal
...
function GetTranRef(pTrancode in number) return varchar is --20.04.2006
  ...
  procedure Save2DetailTable
   ...
   begin
    ...
    insert into tStatementDetailTable values(sBranch, vStateId,sStatementRecNumber,vchContract, vchDeposit, vCardNo,vCardMbr1,
    ...
    Begin
      ...
      for i in (
        ...
        if vChDeposit != ' ' then
          ...
          -----
           begin
              SCH_Customer.GetDueAmount(vChDeposit,vToDate, vDueAmount, vOverDueAmount);
              exception
               when OTHERS then
                 null;
           end;
 ...
 begin
   ...
   for i in (
     ...
     if vDepositCh != ' ' then
       ...
       -----
       begin
         SCH_Customer.GetDueAmount(vDepositCh,vToDate, vDueAmount, vOverDueAmount);
         exception
           when OTHERS then
             null;
       end;
...
procedure SaveDate2MasterTable
  ...
  s.say('END Card for contract '||vChContract,2);
 commit;
 exception
   when others then
