CREATE OR REPLACE PACKAGE BODY sttotabledebit AS
    ...
    ****************************************************************************
    ...
    26.06.2007  Emelyanova M.V.  Was add table tExCommon
...
procedure GetFio
  ...
  Begin
    ...
    GetAccUserAttributes(vDeposit);       --новые реквизиты
  ...
  Begin
    ...
    for i in (
      ...
      if vMoney_Out != 0 or vMoney_In != 0 then
        ...
        if i.Ident in ('OPEXT_A10','OPEXT_B15','OPEXT_P15','OPEXT_B23','OPEXT_V15','OPEXT_V23','ACC_ADD','ACC_SUB') /*and
          vMoney_In = 0*/ then
...
function GetDailyLimit(pPan in varchar,pMbr in number) return number
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableDebit.GetDailyLimit Error:'||sqlErrm);
       return vLimit;
  end;
function GetCashLimit(pPan in varchar,pMbr in number) return number
  is
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableDebit.GetCashLimit Error:'||sqlErrm);
       return vLimit;
  end;
procedure SetWithDrawalForCard(pCardPan in varchar,pCardMBR in number,pWithdrawal in number)
  is
...
-- перенос строки без разрыва слов
  ...
  Begin
    ...
    insert into tStatementDetailTable values(sBranch, vStateId ,sStatementRecNumber, null,vdeposit, vAbbreviation,vCardNo,vCardMbr1,
...
procedure GetAccountLimits(pAccount in varchar)
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableDebit.GetAccountLimits Error:'||sqlErrm);
       null;
  end;
...
procedure SetCardLimits(pCurrPanNo in number, pCardLim out number, pCardAvailLim out number, pCardCashLim out number, pCardAvailCashLim out number)
  ...
  exception
    when others then
      s.say('StToTableDebit.SetCardLimits Error:'||sqlErrm);
      null;
  end;
