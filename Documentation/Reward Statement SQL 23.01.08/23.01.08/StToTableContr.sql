CREATE OR REPLACE PACKAGE BODY StToTableContr AS
    ...
    ****************************************************************************
    ...
    09.04.2007  Arevkina E.      Was modified definition of card limits
    22.05.2007  Arevkina E.N.    Account Available limit [MSCC-314]
    03.08.2007  Емельянова М.В.  Добавлена таблица tExCommon
    21.11.2007  Gimalova A.R.    Adding User Attributes  [MSCC-314]
    22.01.2008  Gimalova A.R.    Adding Bonus Contract Statement [MSCC-1053]
   \****************************************************************************/
  ...
  c_Bonus2Prize      constant number:= ReferenceEntry.GetCode('BONUS2PRIZE');
  c_BonusBurn        constant number:= ReferenceEntry.GetCode('BONUSBURN');
 ...
 -- vText        varchar(4000);
  ...
  vValueEarneed    number;
  vValueRedeemed   number;
  vValueExpired    number;
  vLinkContract    varchar2(100);

  PROCEDURE AddRec(pText in varchar);
  PROCEDURE FormMassiv;
  PROCEDURE GetBeginBalance(pAccount in varchar);
  PROCEDURE GetPaymentReceived;
  PROCEDURE GetNameComp(pAccount in varchar);
 ...
 -- PROCEDURE PrintEndRep;
  ...
  -- Bonus Program
  PROCEDURE FormMassivBonus;
  PROCEDURE GetTransactionsBonus;
  PROCEDURE PrintTitleBonus;
  procedure PrintEndingTrBonus;
  procedure InitNull;
  function  GetLinkContract (pNo in varchar) return varchar;
  ...
  */
    ...
    vBonus      := nvl(Contract.GetAccountNoByItemName(Statementdataprepare.sParameters.Contractno,'ItemBonus'),' ');
...
procedure GetContractData(pContract in varchar)
  ...
  begin
    s.say('!!!!!!!!!!!!! vPageNumber '||vPageNumber,2);
...
procedure PrintProgon
  ...
  Begin
    ...
    if not vcheckmake then
      GetNameComp(vDeposit);
      vdate := vFromDate;
      GetBeginBalance(vDeposit);
...
procedure GetNameComp(pAccount in varchar)
  ...
  Begin
    ...
    Begin
      ...
      a.Acct_Stat, a.createdate,cl.personalcode
      ...
      vAccStat,vAccountCreateDate,vCustomerNo
      from taccount a,tclientpersone cl,treferencecurrency r
      WHERE a.branch = sbranch and
      a.accountno = pAccount and
    ...
    if vIdClient = -1 then --Corporate
      Begin
        ...
        '',null,null, null, '', '',
        a.Acct_Stat, a.createdate,cl.regno
        into vCompany,vFio,vAddressCont,vAccType,vAbbreviation,
        vvTitle,vCountryCont,vRegionCont,vCityCont,vZIPCont,vIdClient,
        vCustomerCreateDate,vCustomerType,
        vPosition,vCompDepatment,vEmployee, vInsider, vVIP,vBlocked,
    ...
    vExternal := GetOldAccNo(pAccount);
    if vinsider = '1' then
    ...
    GetAccUserAttributes(pAccount);       --новые реквизиты
    GetClientUserAttributes(vIdClient);
    --
    vCompanyName := ' ';
    begin
    ...
    /*if i.createdate between vFromDate and vToDate and i.parentpan is not null then
      ...
      for j in 1..sPanCounter loop
        if i.pan = sPanArr(j).Pan then
          if i.ParentPan is not null then
            ...
            sPanArr(j).CardAccountNo  := pAccount;
            sPanArr(j).CardClientName  := i.FIO;
            sPanArr(j).CardPaymentMethod  := GetPayMethod(vContract, pAccount);
      ...
      if vCardFlag = true then
        ...
        sPanArr(sPanCounter).CardAccountNo  := pAccount;
        sPanArr(sPanCounter).CardClientName  := i.FIO;
        sPanArr(sPanCounter).CardPaymentMethod  := GetPayMethod(vContract, pAccount);
 ...
 if not found then
  ...
  Begin
    ...
    for i in (
      ...
      0       Money_In
      from tEntry e,tDocument d,tPoscheque p,tVoiceSlip v,tAtmExt ae, tExCommon ex
      where e.Branch       = sbranch and
            e.Debitaccount = vDeposit and
            e.Value       != 0 and
            d.branch       = e.Branch and
...
procedure GetDailyLimit(pPan in varchar,pMbr in number)-- return number
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableContr.GetDailyLimit Error:'||sqlErrm);
       null;
  --  return vLimit;
  end;
procedure SetWithDrawalForCard(pCardPan in varchar,pCardMBR in number,pWithdrawal in number)
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableContr.SetWithDrawalForCard Error:'||sqlErrm);
       null;
  end;
procedure SetUsageForCard(pCardPan in varchar,pCardMBR in number,pSum in number)
  is
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableContr.SetUsageForCard Error:'||sqlErrm);
       null;
  end;
...
procedure GetAccountLimits(pAccount in varchar)
  ...
  begin
    ...
    if vNew_Balance < 0 and vAccountLim is not null then
      ...
      vAccAvailLim :=  vAccountLim + vNew_Balance;
    end if;
    if vAccCashLim > vAccountLim then         --09.04.2007
      vAccCashLim := vAccountLim;
    end if;
    ...
    if vAccAvailCashLim > vAccAvailLim then
      vAccAvailCashLim := vAccAvailLim;
    end if;
...
procedure SetPanPrinted(pPan in varchar)
 ...
 begin
   ...
   exception
    when OTHERS then
      s.say('StToTableContr.SetPanPrinted Error:'||sqlErrm);
      null;
 end;
procedure PrintUnPrintedCards(pPrimary in number)
 is
...
procedure PrintAddressRec(pRecCounter in number, pRecord in varchar)
  ...
  begin
    ...
    exception
     when OTHERS then
       s.say('StToTableContr.PrintAddressRec Error:'||sqlErrm);
       null;
  end;
procedure PrintAddress(pAddressCont in varchar)
  is
  ...
  Begin
   ...
   -- AddRec(ltrim(vAddressCont));
    ...
    begin
      ...
      exception
        when OTHERS then
          s.say('StToTableContr.GetAvailableLimit Error:'||sqlErrm);
          null;                                       --exep
    end;
...
-- перенос строки без разрыва слов
  ...
  Begin
    ...
    insert into tStatementDetailTable values(sBranch,vStateId, sStatementRecNumber, vContract, vdeposit, vAbbreviation,vCardNo,vCardMbr1,
    ...
    if vPrintTransaction then
      ...
      if vtermlocation != ' ' and vOrgValue != 0 then
        vTextStr := vTermLocation||' Original currency Amount '||
...
procedure PrintTitleBonus
  ...
  Begin
   ...
   -- AddRec(ltrim(vAddressCont));
    ...
    AddRec(' ');
    if vPrintPreBalance then
      AddRec('Transaction for Account '||vBonus);
      if vPrevious_balance > 0 then
        AddRec(Service.rightpad(Service.rightpad(' ',24)||'Previous Balance',87)||to_char(vPrevious_balance,'9999990.00')||' CR');
      else
        AddRec(Service.rightpad(Service.rightpad(' ',24)||'Previous Balance',87)||to_char(Abs(vPrevious_balance),'9999990.00'));
...
procedure GetTransactionsBonus is
    vNumber_One number;
    vFirstline  boolean := true;
    vPreviosPan varchar2(20) := null;
    vPrimaryCard number := 0;
    ...
    for i in (
      ...
      from(
        select
              ...
              e.value Money_Out,
              0       Money_In
        from  tEntry e,tReferenceEntry re,tDocument d
        where e.Branch       = sbranch and
              e.Debitaccount = vBonus and
              e.Value       != 0 and
              re.branch      = e.branch and
              re.code        = e.debitentcode and
              d.branch       = e.Branch and
        ...
        where e.Branch       = sbranch and
              ...
              d.docno        = e.docno and
              d.NewDocNo     is null and
              d.OpDate       between vFromDate and vToDate) Trans
      Group by
              Opdate,
              ValueDate,
              Docno,
  ...
  procedure PrintEndingTrBonus
  is
  cursor cBonusTotal is
    select
      sum (ValueRedeemed) ValueRedeemed,
      sum (ValueExpired)  ValueExpired,
      sum (ValueEarneed)  ValueEarneed
    from(
      select
            decode(e.DebitEntCode, c_Bonus2Prize, 0, decode(e.DebitEntCode, c_BonusBurn, 0, e.value)) ValueRedeemed,
            decode(e.DebitEntCode,c_BonusBurn, e.value, 0) ValueExpired,
            0 ValueEarneed
      from  tEntry e,tReferenceEntry re,tDocument d
      where e.Branch       = sbranch and
            e.Debitaccount = vBonus and
  ...
  Begin
    ...
    AddRec(nvl(vStrText,' '));
    vLinkContract := GetLinkContract(vContract);
    for i in cBonusTotal loop
      vValueEarneed  := i.ValueEarneed;
      vValueRedeemed := i.ValueRedeemed;
      vValueExpired  := i.ValueExpired;
      AddRec('Earned_Bonus     '||to_char(i.ValueEarneed,'9999990.00'));
      AddRec('Redeemed_Bonus   '||to_char(i.ValueRedeemed,'9999990.00'));
      AddRec('Expired_Bonus    '||to_char(i.ValueExpired,'9999990.00'));
      AddRec('Credit contracts '||vLinkContract);
    end loop;
    PrintProgon;
  End;

function  GetLinkContract (pNo in varchar) return varchar is
  vLinkContr varchar2(100);
  begin
    vLinkContr := null;
    for i in
      (
      select a.LinkNo, a.Priority, c.Name, b.Status, a.LinkCode,
                    b.IdCLient
              from tContractLink a, tContract b, tContractType c
            where a.Branch = sBranch and
                  a.MainNo = pNo and
                  a.LinkName = 'BONUS' and
                  -- a.LinkCode = upper(pLinkCode) and
                  b.Branch = a.Branch and
                  b.No = a.LinkNo and
                  c.Branch = b.Branch and
                  c.Type = b.Type
      ) loop
      if vLinkContr is null then
        vLinkContr := i.LinkNo;
      else
        vLinkContr := vLinkContr || ', '||i.LinkNo;
      end if;
    end loop;
    return vLinkContr;
    exception
      when others then
        s.say('StToTableContr.GetLinkContract Error:'||sqlErrm);
        return vLinkContr;
  end;
--------------------------------------
procedure SaveDate2MasterTable
  ...
  begin
    ...
    for i in 1.. sPanCounter loop
      ...
      -- Bonuses
      vValueEarneed, vValueRedeemed, vValueExpired, vLinkContract);
      vStateId := vStateId + 1;
    end loop;
...
procedure SetCardLimits(pCurrPanNo in number, pCardLim out number, pCardAvailLim out number, pCardCashLim out number, pCardAvailCashLim out number)
  ...
  begin
    --CardLimit
    if sPanArr(pCurrPanNo).CardLimit is null or sPanArr(pCurrPanNo).CardLimit > vAccountLim then
      sPanArr(pCurrPanNo).CardLimit := vAccountLim;
    end if;
    ...
    if vCardCashLim is null or vCardCashLim > vAccCashLim then
      vCardCashLim := vAccCashLim;
    end if;
    --CardAvailCashLimit
    ...
    if vCardAvailLim > vAccAvailLim then
      vCardAvailLim := vAccAvailLim;
    end if;
    if vCardAvailCashLim > vAccAvailCashLim then
      vCardAvailCashLim := vAccAvailCashLim;
    end if;
    pCardLim          := vCardLim;
  ...
  exception
    when others then
      s.say('StToTableContr.SetCardLimits Error:'||sqlErrm);
      null;
  end;
