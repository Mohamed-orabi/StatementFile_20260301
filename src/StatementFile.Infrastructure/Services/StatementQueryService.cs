...
{
    /// <summary>
    /// SQL Server implementation of <see cref="IStatementQueryService"/>.
    ///
    ...
    public sealed class StatementQueryService : IStatementQueryService
    {
        ...
        public DataSet LoadWithOverdueDays(int branchCode, string additionalCondition = null)
        {
            ...
            string sql =
                $"SELECT m.*, " +
                $"ISNULL((SELECT eod.ODDAYS FROM A4M.ZM_EOD_CONT_ACCT eod " +
        ...
        public DataSet LoadClientPasNoAndBirthYear(int branchCode)
        {
            ...
            // Oracle EXTRACT(YEAR FROM c.birthday) → T-SQL YEAR(c.birthday)
            // Oracle i."NO" → SQL Server i.[NO]
            string sql =
                $"SELECT c.idclient, i.[NO] AS passportno, " +
                $"YEAR(c.birthday) AS birthyear " +
